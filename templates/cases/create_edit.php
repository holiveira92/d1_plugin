...
if(empty($id_card)){
    $sql                = "INSERT INTO " . $wpdb->prefix . "d1_cases(title_card,desc_card,subtitle_card,text_footer_card,subtext_footer_card,card_link,img_bg_url,
                        detach_card,desc_completa_primaria,desc_completa_secundaria,objetivos_title,objetivos_desc_completa,impactos, desafios, implantacao) 
    ...
    $wpdb->query($wpdb->prepare($sql,array(
        ...
        $data['impactos'],$data['desafios'],$data['implantacao']
    )));
...
//update
    ...
    impactos='%s', desafios='%s', implantacao='%s' WHERE id_card = %d";
    ...
    $data['detach_card'],$data['desc_completa_primaria'],$data['desc_completa_secundaria'],$data['objetivos_title'],$data['objetivos_desc_completa'],
    $data['impactos'],$data['desafios'],$data['implantacao'],$data['id_card'])));
}
