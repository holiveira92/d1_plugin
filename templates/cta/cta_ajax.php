...
foreach($table_data as $key=>&$value){
    if(empty($value['id'])){
        //insert
        $sql                    = "INSERT INTO " . $wpdb->prefix . "d1_call_to_action(title,link,target) VALUES('%s','%s','%s')";
        $wpdb->query($wpdb->prepare($sql,array($value['title'],$value['link'],$value['target'])));
    }else{
        //update
        $sql                    = "UPDATE " . $wpdb->prefix ."d1_call_to_action SET title='%s', link='%s', target='%s' WHERE id = '%s';";
...
if(!empty($ids_delete[0])){
    foreach($ids_delete as $id){
        $wpdb->query($wpdb->prepare("DELETE FROM ". $wpdb->prefix . "d1_call_to_action WHERE id=$id;",array()));
    }
}
