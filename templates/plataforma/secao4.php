...
<body>
    ...
    <form id="faq_fields" action="<?php echo $url_action; ?>">
        <div class="form-style-5" id='secao_content'>
            ...
            <?php
            global $wpdb;
            $faqs = json_decode(json_encode($wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . D1_LANG . 'd1_faq WHERE page = "plataforma" ')), true);
            $cont_faq = 0;
            foreach ($faqs as $key => &$faq) :
                ...
                <fieldset name="d1_faqs" id_faq="<?php echo $faq['id']; ?>">
                    ...
                    <input type="hidden" name="page[]" value="plataforma">
                    ...
                    <button id="remove" type="button" name="remove_faq" id_faq="<?php echo $faq['id']; ?>" class="btn_remove_group">Remover</button>
                </fieldset>
                </div>
            <?php endforeach; ?>
            </div>
