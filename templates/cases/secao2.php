...
<body class="animsition">
    ...
    <div class="table-data__tool">
        <div class="table-data__tool-right">
            <?php   
                    $create_url = "?page=d1_plugin_cases&tab=secao3&";
                    $param = array('path_wp' => ABSPATH, 'id_categoria' => false, 'url_location' => "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
                    $query_string = http_build_query($param); 
            ?>
            <a href="<?php echo $create_url . $query_string ;?>"><button class="button button-primary">
                <i class="zmdi zmdi-plus"></i>Adicionar Categoria</button></a>
        </div>
    </div>
    <div class="table-responsive table-responsive-data2">
        <table class="table table-data2">
            ...
            <tbody>
                <?php 
                    global $wpdb;
                    $result = json_decode(json_encode($wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . 'd1_cases_categorias WHERE id_categoria IS NULL or id_categoria = "" ')),true);
                    $cont = 0;
                    $delete_url = plugins_url('d1_plugin/templates/cases/categorias_delete.php?','d1_plugin');
                    foreach($result as $key=>&$value):
                        $cont++;
                        $create_edit_url = "?page=d1_plugin_cases&tab=secao3&";
                        $param = array('path_wp' => ABSPATH, 'id_categoria' => $value['id'], 'url_location' => "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
                        $query_string = http_build_query($param);
                ?>
                <tr class="tr-shadow">
                    <input type="hidden" name="id_categoria<?php echo D1Plugin::$language; ?>" id="id_categoria" value="<?php echo $value['id'];?>">
                    <td><?php echo $value['descricao'];?></td>
                    <td class="desc"><?php echo $value['descricao'];?></td>
                    <td>
                        <div class="table-data-feature">
                            <a href="<?php echo $create_edit_url . $query_string;?>"><button class="item btn_edit" data-toggle="tooltip" data-placement="top" title="Edit" name="edit<?php echo D1Plugin::$language; ?>">
                                <i class="zmdi zmdi-edit"></i>
                            </button></a>
                            <a href="<?php echo $delete_url . $query_string;?>"><button class="item btn_delete" data-toggle="tooltip" data-placement="top" title="Delete" name="delete<?php echo D1Plugin::$language; ?>">
