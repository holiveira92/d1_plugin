...
class D1_Midia_Fields {
	...
	public function __construct(){
		...
		$this->path_data_fields = plugin_dir_path(dirname_safe(__FILE__,2)) . 'includes/fields/register/d1_midia_fields.json';
		$this->path_data_settings = plugin_dir_path(dirname_safe(__FILE__,2)) . 'includes/fields/register/d1_midia_settings.json';
		$this->active_tab = isset( $_GET[ 'tab' ] ) ? $_GET[ 'tab' ] : 'secao1';
    }

    public function getSettings($opt_group=false,$page='d1_plugin_d1_midia'){
		$settings = array();
    ...
    public function getFields($section=false,$page='d1_plugin_d1_midia'){
		$fields = array();
	...
	/* 	----------------------------------------------------------------------------------------------------------------------------
		...
		----------------------------------------------------------------------------------------------------------------------------
	*/ 
    public function d1_midia_secao1($input){return $input;}
    public function d1_midia_secao2($input){return $input;}
    public function d1_midia_secao3($input){return $input;}
    public function d1_midia_secao4($input){return $input;}
}