...
<body class="animsition">
    ...
    <div class="table-data__tool">
        <div class="table-data__tool-right">
            <?php   
                    $create_url = "?page=d1_plugin_modulos&tab=mod&";
    ...
    <div class="table-responsive table-responsive-data2">
        <table class="table table-data2">
            ...
            <tbody>
                <?php 
                    global $wpdb;
                    $result = json_decode(json_encode($wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "d1_modulos WHERE id_modulo ='' OR id_modulo IS NULL")),true);
                    $cont = 0;
                    $delete_url = plugins_url('d1_plugin/templates/modulos/modulos_delete.php?','d1_plugin');
                    foreach($result as $key=>&$value): 
                        $cont++;
                        $create_edit_url = "?page=d1_plugin_modulos&tab=mod&";
                        $param = array('path_wp' => ABSPATH, 'id_mod' => $value['id'], 'url_location' => "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
                        $query_string = http_build_query($param);
                ?>
                <tr class="tr-shadow">
                    <input type="hidden" name="id_mod<?php echo D1Plugin::$language; ?>" id="id_mod" value="<?php echo $value['id'];?>">
