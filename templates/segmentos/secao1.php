...
<body class="animsition">
    ...
    <div class="table-data__tool">
        <div class="table-data__tool-right">
            <?php   
                    $create_url = "?page=d1_plugin_segmentos&tab=seg&";
            ...
            <a href="<?php echo $create_url . $query_string ;?>"><button class="button button-primary">
                <i class="zmdi zmdi-plus"></i>Adicionar Segmento</button></a>
        </div>
    </div>
    <div class="table-responsive table-responsive-data2">
        <table class="table table-data2">
            <thead>
                <tr>
                    <th width='20%'>Título Principal</th>
                    <th width='20%'>Título Secundário</th>
            ...
            <tbody>
                <?php 
                    global $wpdb;
                    $result = json_decode(json_encode($wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "d1_segmentos")),true);
                    $cont = 0;
                    $delete_url = plugins_url('d1_plugin/templates/segmentos/segmentos_delete.php?','d1_plugin');
                    foreach($result as $key=>&$value): 
                        $cont++;
                        $create_edit_url = "?page=d1_plugin_segmentos&tab=seg&";
                        $param = array('path_wp' => ABSPATH, 'id_seg' => $value['id'], 'url_location' => "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
                        $query_string = http_build_query($param);
                ?>
                <tr class="tr-shadow">
                    <input type="hidden" name="id_seg<?php echo D1Plugin::$language; ?>" id="id_seg" value="<?php echo $value['id'];?>">
    ...
    $(document).ready(function(){
        ...
        $(document).on('click', '.btn_delete', function(){
            if(!confirm('Tem certeza que deseja apagar este Segmento?')){
                return false;
            }
        });
    });
