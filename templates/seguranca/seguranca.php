...
$data_bd            = !empty($id_seg) ? json_decode(json_encode($wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "d1_seguranca WHERE id = '$id_seg'")), true) : array();
$param              = array('path_wp' => ABSPATH, 'id_seg' => $id_seg, 'url_location' => "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
$query_string       = http_build_query($param);
$delete_url         = plugins_url('d1_plugin/templates/seguranca/seg_delete.php?', 'd1_plugin') . $query_string;
$voltar_url         = "?page=d1_plugin_seguranca&tab=secao2&" . $query_string;
$url_action         = plugins_url('d1_plugin/templates/seguranca/seguranca_ajax.php', 'd1_plugin'); 
$data = array(
    ...
    'description_alternative'   => !empty($data_bd[0]["description_alternative"]) ? $data_bd[0]["description_alternative"] : '',
    'url_img'       => !empty($data_bd[0]["url_img"]) ? $data_bd[0]["url_img"] : '',
    'tipo'          => !empty($data_bd[0]["tipo"]) ? $data_bd[0]["tipo"] : 'seguranca',
);
?>
...
<body class="animsition">
    <form id="seguranca_fields" action="<?php echo $url_action; ?>">
        <div class="container">
            <div class="row">
                <div class="col form-style-5" id='secao1_content1' style="padding-bottom:0px!important">
                    ...
                    <div class="col form-style-5 middle">
                        ...
                        <input type="hidden" name="tipo<?php echo D1Plugin::$language; ?>" id="tipo" value="seguranca">
                        ...
                        <label for="description_alternative">Descricao Secundaria:</label> <textarea name="description_alternative<?php echo D1Plugin::$language; ?>" placeholder="Descrição Secundária" rows='7'><?php echo $data['description_alternative']; ?></textarea>
        ...
        <p class="submit">
            <input type="submit" id="seg_submit" class="button button-primary" value="Salvar Alterações" />
        </p>
    </form>

<script>
    $(document).ready(function() {
        $('#seg_submit').click(function() {
            $('#url_location').val(window.location.href);
            var action = $('#seguranca_fields').attr('action');
            $('#seguranca_fields').attr('action', action + "?" + $('#seguranca_fields').serialize());
        });
    });
</script>
</body>