...
//insert
if(empty($id_seg)){
    $sql                = "INSERT INTO " . $wpdb->prefix . D1_LANG . "d1_seguranca(title,description,description_alternative,url_img,tipo) 
                            VALUES('%s','%s','%s','%s','%s')";
...
//update
    ...
    $query_string       = http_build_query($param);
    $location           = urldecode($_REQUEST["admin_url"] . "admin.php?page=d1_plugin_seguranca&tab=$page_type&" . $query_string);
    $sql                = "UPDATE " . $wpdb->prefix . D1_LANG ."d1_seguranca SET title='%s', description='%s', description_alternative='%s', url_img='%s', tipo='%s' WHERE id = %d";
