...
//insert
if(empty($id_cargo)){
    $sql           = "INSERT INTO " . $wpdb->prefix . "d1_cargos(title,subtitle,description1,description2,description3,id_departamento) 
                    VALUES('%s','%s','%s','%s','%s','%s')";
...
//update
    ...
    $query_string       = http_build_query($param);
    $location           = urldecode($_REQUEST["admin_url"] . "admin.php?page=d1_plugin_departamentos&tab=cargos&" . $query_string);
    $sql                = "UPDATE " . $wpdb->prefix ."d1_cargos SET title='%s', subtitle='%s', description1='%s', description2='%s',
