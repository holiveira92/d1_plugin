...
<body>
    <?php $url_action = plugins_url('d1_plugin/templates/faq/faq_ajax.php','d1_plugin'); ?>
    <form id="faq_fields" action="<?php echo $url_action; ?>">
        <div class="form-style-5" id='secao_content'>
            ...
            <?php
            global $wpdb;
            $faqs = json_decode(json_encode($wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . 'd1_faq WHERE page = "plataforma" ')), true);
            $cont_faq = 0;
            foreach ($faqs as $key => &$faq) :
                ...
                <fieldset name="d1_faqs<?php echo D1Plugin::$language; ?>" id_faq="<?php echo $faq['id']; ?>">
                    ...
                    <input type="hidden" name="page[]<?php echo D1Plugin::$language; ?>" value="plataforma">
                    <label for="pergunta">Pergunta:</label> <textarea name="question[]<?php echo D1Plugin::$language; ?>" placeholder="Pergunta"><?php echo $faq['question']; ?></textarea>
                    <label for="pergunta">Resposta:</label> <textarea name="answer[]<?php echo D1Plugin::$language; ?>" placeholder="Resposta"><?php echo $faq['answer']; ?></textarea>
                    <button type="button" name="remove_faq<?php echo D1Plugin::$language; ?>" id_faq="<?php echo $faq['id']; ?>" class="btn btn-danger btn_remove_group">Remover</button>
                </fieldset>
                </div>
            <?php endforeach; ?>
            </div>
        ...
        $(document).ready(function() {
            ...
            $('#add_group').click(function() {
                ...
                if (cont > 0) {
                    ...
                    '<input type="hidden" name="page[]<?php echo D1Plugin::$language; ?>" value="plataforma">' + 
                    '<label for="pergunta">Pergunta:</label> <textarea name="question[]<?php echo D1Plugin::$language; ?>" placeholder="Pergunta"></textarea>' +
                    '<label for="pergunta">Resposta:</label> <textarea name="answer[]<?php echo D1Plugin::$language; ?>" placeholder="Resposta"></textarea>' +
                    '<button type="button" name="remove_faq<?php echo D1Plugin::$language; ?>" id_faq="" class="btn btn-danger btn_remove_group">Remover</button>' +
                    '</fieldset> </div>'
                    ).end();
                }
            });
